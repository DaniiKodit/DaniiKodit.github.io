...
const I18N = {
  ru: {
    ...
    'form.consent': 'Отправляя форму, вы даёте согласие на обработку персональных данных (152-ФЗ).',
  ...
  en: {
    ...
    'form.consent': 'By submitting this form you consent to the processing of your personal data (Russian Federal Law 152-FZ).',
...
function initStars() {
  ...
  function resize() {
    ...
    stars = [];
    const n = Math.min(160, Math.round((W * H) / 260));
    for (let i = 0; i < n; i++) {
      stars.push({
        x: (Math.random() * W) | 0,
        y: (Math.random() * H) | 0,
